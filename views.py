...
def play(playcount,gesture):
    class PlayThread(Thread):
        ...
        def run(self):
            ...
            t1.save("audio/"+str(self.playcount)+".mp3")
            playsound("audio/"+str(self.playcount)+".mp3")


    newthread = PlayThread(playcount,gesture)
    newthread.start()

def speakAudio(request):
        if request.method == 'GET':
                global textoutput
                global playcount
                print("got "+textoutput)
                if len(textoutput) > 0:
                        play(playcount,textoutput)
                        playcount = playcount + 1
                return HttpResponse(textoutput)        
